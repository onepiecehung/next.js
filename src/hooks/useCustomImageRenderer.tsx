 ...
 */
export function useCustomImageRenderer(containerRef: React.RefObject<HTMLElement | null>) {
  useEffect(() => {
    if (!containerRef.current) return;

    const processCustomImages = () => {
      const customImageWrappers = containerRef.current?.querySelectorAll('.custom-image-next-wrapper[data-custom-image-src]');
      ...
      customImageWrappers.forEach((wrapper) => {
        ...
        const width = parseInt(wrapper.getAttribute('data-custom-image-width') || "800");
        const height = parseInt(wrapper.getAttribute('data-custom-image-height') || "600");

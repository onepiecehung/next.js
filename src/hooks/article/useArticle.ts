...
function shouldRetry(error: Error): boolean {
  ...
  if (typeof maybe.isRetryable === "boolean") {
    if (!maybe.isRetryable) {
      console.warn(
        `Non-retryable error (${maybe.status}):`,
        maybe.message ?? error.message,
      );
      return false;
    }
    return true;
...
export function useArticle(articleId: string, options?: UseArticleOptions) {
  ...
  const onSuccessRef = useRef<UseArticleOptions["onSuccess"]>(
    options?.onSuccess,
  );
    ...
    async (isRetry = false) => {
      ...
      try {
        ...
        if (retryCountRef.current < maxRetries) {
          ...
          console.warn(
            `Article fetch failed (attempt ${retryCountRef.current}/${maxRetries}), retrying in ${delay}ms...`,
          );
          ...
          console.error(
            `Article fetch failed after ${maxRetries} attempts:`,
            error,
          );
          setError(error);
          setIsLoading(false);
